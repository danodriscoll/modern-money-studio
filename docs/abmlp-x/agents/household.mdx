...
In every step, Households think about the rate of return (ERrbL) they expect to see in the next period (step). One third of Households created will predict a higher ERrbL 33 percent of the time (on average), a lower ERrbL 33 percent of the time (on average) leaving the default expectation of no change in the next step. A third of Households will factor the base rate that has been set for the previous five steps into their expectations logic. Another third of Households have access to both previous rates and logic that employs model (macro) system *velocity* and *acceleration*, that is, the rate at which the Government has been issuing new bills as a percentage of national income.
